...
export function MobileUploadPage() {
  const apiBase = getRuntimeEnv('VITE_API_BASE_URL', '/api/v1')
  const cloudinaryCloudName = getRuntimeEnv('VITE_CLOUDINARY_CLOUD_NAME', 'demo')
  const cloudinaryUploadPreset = getRuntimeEnv('VITE_CLOUDINARY_UPLOAD_PRESET', 'ml_default')
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiBase}/public/document-sessions/${sessionId}`, { params: { t: token } })
      .then((res) => {
      ...
      .catch(() => {
        setStatus('error')
        setError('Phiên tải tài liệu không tồn tại hoặc đã hết hạn.')
      })
  ...
  const handleCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await axios.post(
        `${apiBase}/public/document-sessions/${sessionId}/upload`,
        null,
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        {status === 'expired' && (
          <div className="rounded-lg bg-yellow-50 p-4 text-center">
            <p className="text-sm font-medium text-yellow-800">Phiên đã hết hạn.</p>
