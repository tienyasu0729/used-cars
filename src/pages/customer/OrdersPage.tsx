...
export function OrdersPage() {
  ...
  const visibleVehicleIds = useMemo(
    () =>
      Array.from(
        new Set(
          paginated
            .map((order) => Number(order.vehicleId))
  ...
  const vehicleQueries = useQueries({
    queries: visibleVehicleIds.map((vehicleId) => ({
  ...
  const vehicleById = useMemo(() => {
    const entries: Array<[string, Vehicle]> = []
    visibleVehicleIds.forEach((vehicleId, index) => {
      const vehicle = vehicleQueries[index]?.data
      if (vehicle) {
        entries.push([String(vehicleId), vehicle])
      }
    })
    return new Map(entries)
  }, [visibleVehicleIds, vehicleQueries])
  ...
  const buildExcelRows = (items: typeof orders) =>
    items.map((o) => {
      const v = vehicleById.get(String(o.vehicleId))
      const status = statusMap[o.status]?.label ?? o.status
      return [o.orderNumber ?? String(o.id), v?.title ?? o.vehicleTitle ?? 'Xe', String(o.price), formatDate(o.createdAt), status]
    })
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-200">
              {paginated.map((o) => {
                const vehicle = vehicleById.get(String(o.vehicleId))
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-14 w-20 shrink-0 overflow-hidden rounded-lg border border-slate-100 bg-slate-100">
                          <FallbackImage
                            sources={[getVehicleImageFromImages(vehicle?.images), o.vehicleImageUrl]}
                            fallback="https://placehold.co/160x112?text=No+Image"
                            alt={vehicle?.title ?? o.vehicleTitle ?? 'Xe'}
                            className="h-full w-full object-cover"
                          />
                        </div>
                        <div>
                          <p className="text-sm font-bold leading-none text-slate-900">
                            {vehicle?.title || o.vehicleTitle || (vehicle ? `${vehicle.brand ?? ''} ${vehicle.model ?? ''}`.trim() : 'Xe')}
                          </p>
