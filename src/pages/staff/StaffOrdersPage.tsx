...
function vehicleThumb(v: Vehicle | null | undefined) {
  return getVehicleImageFromImages(v?.images)
}
...
function OrderDetailModal({
  ...
  const thumbSources = [vehicleThumb(vehicle), order.vehicleImageUrl]
          ...
          <div className="flex items-center gap-3">
            <div className="h-12 w-12 overflow-hidden rounded bg-slate-200">
              <FallbackImage
                ...
                alt={order.vehicleTitle || 'Xe'}
                className="h-full w-full object-cover"
              />
            </div>
            <p className="font-semibold text-slate-900">
              {vehicle?.title?.trim() || order.vehicleTitle || (vehicle ? `${vehicle.brand} ${vehicle.model}` : order.vehicleId)}
            </p>
          </div>
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-200">
              {paginated.map((o) => {
                ...
                const rowThumbSources = [vehicleThumb(vehicle), o.vehicleImageUrl]
                return (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 overflow-hidden rounded bg-slate-200">
                          <FallbackImage
                            sources={rowThumbSources}
                            fallback="https://placehold.co/80x60?text=No+Image"
                            alt={o.vehicleTitle || 'Xe'}
                            className="h-full w-full object-cover"
                          />
                        </div>
                        <span className="text-sm font-medium">
                          {vehicle?.title?.trim() || o.vehicleTitle || `${vehicle?.brand ?? ''} ${vehicle?.model ?? ''}`.trim() || 'Xe'}
                        </span>
                      </div>
                    </td>
