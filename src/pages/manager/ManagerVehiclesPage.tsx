...
export function ManagerVehiclesPage() {
  ...
  const managerQuery = useManagerVehicles({
    scope,
    page: page - 1,
    size: pageSize,
    q: debouncedSearch || undefined,
    status: statusFilter || undefined,
    excludeStatus: splitInventory && !statusFilter ? 'InTransfer' : undefined,
    sort: activeTab === 'NETWORK' ? 'postingDateDesc' : 'idDesc',
  })
  const vehicles = managerQuery.items
  const isLoading = managerQuery.isLoading
  const refetch = managerQuery.refetch
  const totalElements = managerQuery.totalElements
  const totalPages = Math.max(1, managerQuery.totalPages || 1)
  ...
  const outgoingQuery = useManagerVehicles({
    page: 0,
    size: 50,
    q: debouncedSearch || undefined,
    status: 'InTransfer',
  ...
  const outgoingList = splitInventory ? outgoingQuery.items : []
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {exportSelectMode && (
        <ExportSelectionBar
          ...
          onSelectAll={() => setExportSelectedIds(new Set(paginated.map((v) => v.id)))}
          onDeselectAll={() => setExportSelectedIds(new Set())}
          onExport={() => {
            const items = paginated.filter((v) => exportSelectedIds.has(v.id))
      ...
      <Pagination page={page} totalPages={totalPages} total={totalElements} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(s) => { setPageSize(s); setPage(1) }} label="xe" />
      <TransferDetailModal
