...
export function BranchMap({ branches, zoom }: BranchMapProps) {
  ...
  const goongMaptilesKey = getRuntimeEnv('VITE_GOONG_MAPTILES_KEY')
  ...
  useEffect(() => {
    const container = mapContainerRef.current
    if (!container || !goongMaptilesKey || !containerVisible) return
  ...
  }, [branchKey, containerVisible, goongMaptilesKey, zoom])
