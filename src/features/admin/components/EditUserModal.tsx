...
export function EditUserModal({
  ...
  const branchSelectClass = readOnly
  ...
  const onFormSubmit = form.handleSubmit(async (data) => {
    if (!user || readOnly || !onSave) return
  ...
  const handleDeactivate = async () => {
    if (readOnly || !onDeactivate) return
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={readOnly ? 'Chi tiết người dùng' : 'Chỉnh sửa người dùng'}
      footer={
        readOnly ? (
