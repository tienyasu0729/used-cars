...
export interface VehicleListingFacetsResponse {
  categoryIds: number[]
...
export const vehicleService = {
  ...
  getVehicleById: async (id: number | string): Promise<Vehicle> => {
    const token = encodeURIComponent(String(id).trim())
    const res = await axiosInstance.get<{ data: unknown }>(`/vehicles/${token}`)
  ...
  getVehicleListingFacets: async (): Promise<VehicleListingFacetsResponse> => {
    const res = await axiosInstance.get<{ data: VehicleListingFacetsResponse }>('/vehicles/facets')
    const apiRes = res as unknown as { data?: VehicleListingFacetsResponse }
    const raw = apiRes.data ?? (res as unknown as VehicleListingFacetsResponse)
    return {
      categoryIds: Array.isArray(raw?.categoryIds) ? raw.categoryIds : [],
      subcategoryIdsByCategory:
        raw?.subcategoryIdsByCategory && typeof raw.subcategoryIdsByCategory === 'object'
          ? raw.subcategoryIdsByCategory
          : {},
      priceMin: Number(raw?.priceMin ?? 0),
      priceMax: Number(raw?.priceMax ?? 1),
    }
  },
}
